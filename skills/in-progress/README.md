...
- **[orchestrate-afk](./orchestrate-afk/SKILL.md)** — Drive a dependency-ordered backlog to merged PRs with a fleet of away-from-keyboard `claude -p` workers, one git worktree each, throttled and gated on green CI or human review. The no-code sibling of Sandcastle. User-invoked.
